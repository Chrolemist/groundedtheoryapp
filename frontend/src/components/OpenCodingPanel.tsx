...
function CodeRow({
  ...
  const updateLabel = useYjsMapText({
    ydoc,
    mapName: 'codes',
    itemId: code.id,
    field: 'label',
    onLocalUpdate: (value) => onUpdateCode(code.id, { label: value }),
  })
      ...
      <div className="space-y-2">
        {codes.map((code) => (
          <CodeRow
            ...
            code={code}
            memos={memos}
            isActive={activeMemoCodeId === code.id}
            showMemos={showMemos}
            ydoc={ydoc}
            ...
            onToggleActive={() =>
              setActiveMemoCodeId((current) => (current === code.id ? null : code.id))
            }
          />
        ))}
        {!codes.length && (
